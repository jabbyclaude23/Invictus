export const EXERCISES = [
  ...
  { name:"Overhead Tricep Ext",     muscle:"arms",      sets:3, reps:"10-12",  desc:"Dumbbell overhead extension",      alts:["Skull Crusher","Tricep Pushdown","Dips"] },
  ...
  // CARDIO
  { name:"Treadmill Run",           muscle:"cardio",    sets:1, reps:"20 min", desc:"Moderate steady-state cardio",     alts:["Bike","Rowing Machine","Jump Rope"] },
  { name:"Bike",                    muscle:"cardio",    sets:1, reps:"20 min", desc:"Stationary bike cardio",           alts:["Treadmill Run","Rowing Machine","Stair Climber"] },
  { name:"Rowing Machine",          muscle:"cardio",    sets:1, reps:"15 min", desc:"Full-body low-impact cardio",      alts:["Bike","Treadmill Run","Jump Rope"] },
  { name:"Jump Rope",               muscle:"cardio",    sets:4, reps:"3 min",  desc:"High-intensity skipping",          alts:["Treadmill Run","Bike","Burpees"] },
  { name:"Stair Climber",           muscle:"cardio",    sets:1, reps:"20 min", desc:"Stair machine cardio",             alts:["Treadmill Run","Bike","Rowing Machine"] },
];

// ─── Weight Recommendation ───────────────────────────────────────────────────
// Base % of bodyweight for an intermediate male (1.0 = bodyweight)
const WEIGHT_PCT = {
  "Bench Press":             { pct: 0.65 },
  "Dumbbell Press":          { pct: 0.22, perArm: true },
  "Incline Press":           { pct: 0.55 },
  "Incline Dumbbell Press":  { pct: 0.20, perArm: true },
  "Cable Fly":               { pct: 0.10 },
  "Dumbbell Fly":            { pct: 0.10, perArm: true },
  "Pec Deck":                { pct: 0.35 },
  "Lat Pulldown":            { pct: 0.50 },
  "Barbell Row":             { pct: 0.65 },
  "Dumbbell Row":            { pct: 0.28, perArm: true },
  "Cable Row":               { pct: 0.45 },
  "Deadlift":                { pct: 1.20 },
  "Trap Bar Deadlift":       { pct: 1.20 },
  "Straight-Arm Pulldown":   { pct: 0.15 },
  "Overhead Press":          { pct: 0.45 },
  "Dumbbell Shoulder Press": { pct: 0.18, perArm: true },
  "Arnold Press":            { pct: 0.18, perArm: true },
  "Lateral Raise":           { pct: 0.07, perArm: true },
  "Cable Lateral Raise":     { pct: 0.07 },
  "Face Pull":               { pct: 0.15 },
  "Rear Delt Fly":           { pct: 0.07, perArm: true },
  "Squat":                   { pct: 1.00 },
  "Goblet Squat":            { pct: 0.25 },
  "Romanian Deadlift":       { pct: 0.70 },
  "Leg Press":               { pct: 1.50 },
  "Hack Squat":              { pct: 0.80 },
  "Leg Curl":                { pct: 0.40 },
  "Leg Extension":           { pct: 0.40 },
  "Bulgarian Split Squat":   { pct: 0.20, perArm: true },
  "Lunges":                  { pct: 0.20, perArm: true },
  "Calf Raise":              { pct: 0.80 },
  "Seated Calf Raise":       { pct: 0.60 },
  "Barbell Curl":            { pct: 0.28 },
  "Dumbbell Curl":           { pct: 0.12, perArm: true },
  "Hammer Curl":             { pct: 0.12, perArm: true },
  "Cable Curl":              { pct: 0.20 },
  "Tricep Pushdown":         { pct: 0.25 },
  "Skull Crusher":           { pct: 0.30 },
  "Overhead Tricep Ext":     { pct: 0.15, perArm: true },
  "Close-Grip Bench Press":  { pct: 0.55 },
};

const EXP_MULT   = { "Beginner (< 1 year)": 0.55, "Intermediate (1-3 years)": 1.0, "Advanced (3+ years)": 1.50 };
const SEX_MULT   = { "Male": 1.0, "Female": 0.65 };
const BODYWEIGHT = new Set(["Pull-Up","Push-Up","Dips","Plank","Dead Bug","Hanging Leg Raise","Russian Twist","Ab Wheel"]);

/**
 * Returns { display: "~60 kg", value: 60, unit: "kg" }
 * or null if we can't calculate.
 */
export function getWeightRec(exerciseName, profile) {
  if (!profile?.weight) return null;
  if (BODYWEIGHT.has(exerciseName)) return { display: "Bodyweight", value: null, unit: null };

  const rec = WEIGHT_PCT[exerciseName];
  if (!rec) return null;

  const bw  = parseFloat(profile.weight); // always kg (stored metric)
  const exp = EXP_MULT[profile.experience] || 0.55;
  const sex = SEX_MULT[profile.sex]        || 1.0;

  let kg = bw * rec.pct * exp * sex;
  kg = Math.max(5, Math.round(kg / 2.5) * 2.5); // round to nearest 2.5 kg, min 5

  // Display in user's preferred unit
  const useKg = !profile.weight_display?.includes("lbs");
  if (useKg) {
    const suffix = rec.perArm ? " kg / arm" : " kg";
    return { display: `~${kg}${suffix}`, value: kg, unit: "kg" };
  } else {
    const lbs = Math.round(kg * 2.20462 / 5) * 5; // round to nearest 5 lbs
    const suffix = rec.perArm ? " lb / arm" : " lb";
    return { display: `~${lbs}${suffix}`, value: lbs, unit: "lb" };
  }
}
...
export const MUSCLE_GRADIENT = {
  chest:     "from-red-900/60 to-red-700/20",
  back:      "from-blue-900/60 to-blue-700/20",
  shoulders: "from-purple-900/60 to-purple-700/20",
  legs:      "from-green-900/60 to-green-700/20",
  arms:      "from-yellow-900/60 to-yellow-700/20",
  core:      "from-orange-900/60 to-orange-700/20",
  cardio:    "from-cyan-900/60 to-cyan-700/20",
};

export const MUSCLE_ICON = {
  chest: "💪", back: "🏋️", shoulders: "🎯", legs: "🦵",
  arms: "💪", core: "⚡", cardio: "🏃",
};

export const getExercise    = name => EXERCISES.find(e => e.name === name);
export const getAlternatives = ex  => (ex?.alts || []).map(n => EXERCISES.find(e => e.name === n)).filter(Boolean);
// Returns all library exercises with same muscle, excluding the current one
export const getSameMuscleAlts = (muscle, excludeName) =>
  EXERCISES.filter(e => e.muscle === muscle && e.name !== excludeName);
